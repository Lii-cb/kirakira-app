function myFunction() {
    ...
    // サービスアカウントキー (JSON)
    // NOTE: Replace this with your actual JSON key content if it's different/reset
...
`;
    ...
    function getFirestore() {
        // 改行コードの正規化（念のため）
        const jsonString = PRIVATE_KEY_JSON.trim();
    ...
    function syncFromFirestore() {
        ...
        try {
            ...
            for (const doc of allDocs) {
                ...
                // おやつ免除フラグの取得 (Map型の中にある booleanValue)
                let isExempt = "";
    ...
    function syncToFirestore() {
        ...
        try {
            ...
            for (let i = 0; i < values.length; i++) {
                ...
                const isExemptInput = row[6].toString().trim(); // "○" or ""

                if (!name) continue;

                const isExempt = isExemptInput === "○" || isExemptInput === "TRUE" || isExemptInput === "true";
