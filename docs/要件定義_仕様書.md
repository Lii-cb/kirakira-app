...
| 入金 | 読み書き | **読取のみ** | 自分の子のみ作成/読取 | |
| 文書 | 読み書き | **読取のみ** | 読取のみ | |
| 職員勤怠 | 読み書き | 読み書き | ❌ | |
| システム設定 | 読み書き | **読取のみ** | 読み取り | |
| 職員マスター | 読み書き | **読取のみ** | ❌ | |
| 保護者マスター | 読み書き | **読取のみ** | 自分のみ読取 | |

### 3.5 管理者/職員モード切替
管理画面には **「Admin / Staff」モード切替トグル** がある。
- **Admin モード:** 全メニュー表示（ダッシュボード、カレンダー、児童管理、職員管理、金銭管理、おたより管理、設定）
- **Staff モード:** ダッシュボード表示。日付選択は自由（当日固定ではない）
- モード切替時にPIN入力を要求（PINは `system_settings/current.adminPin` に保存。設定画面またはスプレッドシートのSettingsシートから変更可能）
...
| 機能 | 詳細 |
|------|------|
| **日付選択** | カレンダーポップオーバーで日付選択（Admin/Staff共に自由選択可） |
...
| 機能 | 詳細 |
|------|------|
| **出勤/退勤操作** | ステータス切替: 出勤中 → 外出中 → 退勤 → 欠勤 → シフト予定 |
| **時間編集** | シフト時間・出勤/退勤時間を個別に編集可能 |
| **メモ機能** | 当日の業務メモを保存（日報用） |
| **リアルタイム更新** | Firestore `onSnapshot` による職員ステータス自動更新 |

> **注:** 職員の追加・削除・シフト一括登録はダッシュボードパネルからは除去。職員管理ページまたはスプレッドシートで実施。
...
| 機能 | 詳細 |
|------|------|
| **一覧表示** | 全児童をテーブル表示（ID、学年、氏名/かな、連絡先、許可メール、帰宅方法、おやつ設定） |
...
| # | 列ヘッダ | 必須 | 説明 |
|---|----------|------|------|
| 1 | ID | ❌ | ドキュメントID（空なら自動採番） |
| 2 | 学年 | ✅ | 1-6 |
| 3 | 氏名 | ✅ | 児童名 |
| 4 | かな | ✅ | フリガナ |
| 5 | 帰宅方法 | ❌ | デフォルト帰宅方法 |
| 6 | 許可メール | ❌ | 保護者Googleメール（セミコロン区切りで複数可） |
| 7 | 電話番号 | ❌ | 連絡先（セミコロン区切りで複数可） |
| 8 | おやつ免除 | ❌ | 1=免除、0=あり |
...
| **管理者モード切替PIN** | 4桁以上の数字。管理者モード切替時に要求 |
...
| 成功時遷移 | → `/parent/home` |
| ナビゲーション | ヘッダーに「ホーム」「利用予約」「利用料金」「おたより」 |
...
| **時間帯選択** | 「通常 15:00-17:00」「15:00-17:30」「15:00-18:00」「15:00-18:30」の4択 |
| **おやつ選択** | チェックボックス（おやつ不要設定の児童は自動OFF） |
| **料金見積** | おやつ料(100円/日) × 日数 を自動計算表示（基本料・延長料なし） |
...
> **注:** `className`, `address`, `parentName` はPII削減のため削除済み。出席レコードの `className` は `grade年` 形式で非正規化。
...
| `fees` | object | ✅ | `{ basePrice, snackPrice }` |
| `notifications` | object | ❌ | `{ emailEnabled }` |
| `features` | object | ❌ | `{ newReservationsEnabled }` |
| `adminPin` | string | ❌ | 管理者モード切替PIN（4桁以上） |
...
- isAdmin(): staff_usersドキュメントのrole == "admin" を検証
- payments/documents/staff_users/staff/parents/system_settings の write → isAdmin() のみ
